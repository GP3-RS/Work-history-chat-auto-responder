        ...
        try {
          ...
          try {
            if (process.env.CACHE === "Redis")
              ...
              cacheResults = (await cache.get(data.question)).props?.value;
          } catch (err) {
          ...
          if (!cacheResults) {
            ...
            if (!responseObj) {
              console.log(
                "No response from openai.generate response invocation"
              );
