            ...
            switch (_a.label) {
                case 0:
                    if (process.env.ENV === "Render" && process.env.REDIS_URL)
                        ...
                        cache = redis.createClient();
                    cache.on("error", function (error) { return console.error("Redis client error: ".concat(error)); });
                    return [4 /*yield*/, cache
                            .connect()
                            .then(function () { return console.log("Connected to ".concat(process.env.ENV, " Redis cache")); })];
                case 1:
                    _a.sent();
                    return [2 /*return*/];
            }
